...
testSites = open('globalTestSites.geojson') # Real test sites file
# fakeTestSites = open('globalTestSites.geojson') # Fake test sites file used for testing purposes
sites = json.load(testSites)['features']
...
for server in servers:
    ...
    try:
        ...
        for site in sites:
            ...
            print(site['properties']['state'] )

            if (site['properties']['state'] is not None):
...
#                             printOut("Flow Statistic not equal: " + prodResult['code'])
#                             printOut("TEST value = " + str(testDictionary[prodStatisticGroup['StatisticGroupID']][prodRegressionRegion['Code']][prodResult["code"]] ))
#                             printOut(productionServer + " value = " + str(prodResult['value']))
